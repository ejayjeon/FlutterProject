"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AuthController = void 0;
const common_1 = require("@nestjs/common");
const auth_service_1 = require("./auth.service");
const basic_token_guard_1 = require("./basic-token.guard");
const bearer_token_guard_1 = require("./bearer-token.guard");
const user_service_1 = require("../user/user.service");
const swagger_1 = require("@nestjs/swagger");
const api_bearer_token_header_1 = require("../core/decorator/api-bearer-token-header");
let AuthController = class AuthController {
    constructor(authService, userService) {
        this.authService = authService;
        this.userService = userService;
    }
    async login(req) {
        return this.authService.login(req.user);
    }
    async token(req) {
        return {
            accessToken: await this.authService.rotateAccessToken(req.token),
        };
    }
};
__decorate([
    (0, swagger_1.ApiOperation)({
        summary: 'Login하기',
    }),
    (0, api_bearer_token_header_1.ApiBasicTokenHeader)(),
    (0, swagger_1.ApiOkResponse)({
        description: 'Access Token과 Refresh Token',
        schema: {
            properties: {
                accessToken: {
                    type: 'string',
                    example: 'asdiofjzxl;ckvjoiasjewr.asdfoiasjdflkajsdf.asdfivjiaosdjf',
                    description: 'Access Token',
                },
                refreshToken: {
                    type: 'string',
                    example: 'asdiofjzxl;ckvjoiasjewr.asdfoiasjdflkajsdf.asdfivjiaosdjf',
                    description: 'Refresh Token',
                },
            },
        },
    }),
    (0, common_1.UseGuards)(basic_token_guard_1.BasicTokenGuard),
    (0, common_1.Post)('login'),
    __param(0, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], AuthController.prototype, "login", null);
__decorate([
    (0, swagger_1.ApiOperation)({
        summary: 'Token Refresh하기',
    }),
    (0, api_bearer_token_header_1.ApiBearerTokenHeader)(),
    (0, swagger_1.ApiOkResponse)({
        description: 'Access Token',
        schema: {
            properties: {
                accessToken: {
                    type: 'string',
                    example: 'asdiofjzxl;ckvjoiasjewr.asdfoiasjdflkajsdf.asdfivjiaosdjf',
                    description: 'Access Token',
                },
            },
        },
    }),
    (0, common_1.UseGuards)(bearer_token_guard_1.RefreshTokenGuard),
    (0, common_1.Post)('token'),
    __param(0, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], AuthController.prototype, "token", null);
AuthController = __decorate([
    (0, swagger_1.ApiTags)('auth'),
    (0, common_1.Controller)('auth'),
    __metadata("design:paramtypes", [auth_service_1.AuthService,
        user_service_1.UserService])
], AuthController);
exports.AuthController = AuthController;
//# sourceMappingURL=auth.controller.js.map