...
from gongmore.models import Gongmore
# 고정부분
...
# delete Gongmos 
row, _ = Gongmore.objects.filter(date__lte=month).delete()
print(row, "Gongmos deleted")
...
for gongmo_list in gongmo_lists:
    ...
    if(int(gongmo_date[4:5]) == month):
        if(Gongmore.objects.filter(title__iexact=gongmo_title).count() == 0):
            Gongmore(
                title=gongmo_title, price=gongmo_price, part=gongmo_part, host=gongmo_host, date=gongmo_date, listdate=gongmo_listdate
            ).save()
